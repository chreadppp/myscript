...
K3S_TOKEN=
read -p  "master节点的IP(例如：192.168.1.10)： " MASTER_K3S_IP
echo "默认master端口6443。"
...
INSTALL_K3S_EXEC="agent --disable=traefik  --docker --kube-apiserver-arg service-node-port-range=20000-40000  --write-kubeconfig ~/.kube/config --write-kubeconfig-mode 666"
